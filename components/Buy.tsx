...
export default function Buy() {
  ...
  const [slideshowOnly, setSlideshowOnly] = useState(query.slideshowOnly)
  ...
  const salesContract = useContract(contractAddresses.salesFactory[chainId], contractAddresses.salesAbi, true)
  ...
  const getContractStates = async () => {
    let token_id: any = tokenId.toString()
    if (!salesIds.includes(token_id)) {
      location.href = "https://emblem.finance/nft?id="+token_id
    }
    console.log("Contract states")
    let cost = await salesContract.costs(tokenId)
    setCost(cost)
    let balance = await salesContract.balances(tokenId)
    setBalance(balance.toNumber())
    let sold = await salesContract.minted(tokenId)
    setSold(sold.toNumber())
    let _newTokenId = Number(tokenId) + Number(sold) + 1
    setNewTokenId(_newTokenId)
    // let owned = false
    // let _owner
    // try {
  ...
  const buyVault = async () => {
    ;(salesContract as Contract)
    .buy(tokenId, {value: cost})
    .then(({ hash }: { hash: string }) => {
      setHash(hash)
    })
    .catch((error: ErrorWithCode) => {})
      ...
      {/* <Loader loaded={state.loaded}> */}
              ...
              >
                ...
                <Stack className="NFT-content" align="center">
                  { vaultValues.length && vaultValues.filter(item=> {return item.type == "nft"}).length > 0 ? (
                      <NFTSlideshow name={vaultName} image={vaultImage} items={[]} properties = {{'duration': 3000, canSwipe: false}}/>
                    ) : (
                ...
                <Stack mb={10} direction="column" align="center">
                  <Button mb={5} onClick={buyVault}>Buy Vault</Button>
                  <Text>Cost: {(Number(cost) * Math.pow(10, -18)).toFixed(5)}</Text>
                  <Text>Remaining for sale: {balance} </Text>
                  <Text>Sold: {sold}</Text>
                  {/* <Text>New TokenId: {newTokenId}</Text> */}
        ...
        {hash ? (
          <TransactionToast
            hash={hash}
            onComplete={() => {
              location.href = location.origin + '/nft?id='+newTokenId
            }}
          />
        ) : null}
        <Box height="40px"></Box>
