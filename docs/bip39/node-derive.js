let libs = {bitcoin: require("bitcoinjs-lib")} 
// let foo = require("./bitcoinjs-extensions.js")

let mnemonic = require("bip39")
libs.bitcoin.HDNode = require('bip32')
libs.ethUtil = require('ethereumjs-util')
libs.bchaddr = require('bchaddrjs')

function phraseToKey(phrase, coinValue){
    var seed = mnemonic.mnemonicToSeedSync(phrase, '');
    let networkName = 
    ...
    var bip32RootKey = libs.bitcoin.HDNode.fromSeed(Buffer.from(seed), network);
    console.log('bip32RootKey', bip32RootKey.toBase58())

    return calculateAddressAndKey(coinValue, bip32RootKey, network);
}

function calculateAddressAndKey(coinValue, bip32RootKey, network) {
    ...
    var keyPair = key
    var address = libs.bitcoin.payments.p2pkh({ pubkey: key.publicKey, network: network}).address
    var privkey = keyPair.toWIF();
    var pubkey = key.publicKey.toString('hex')
    if (coinValue == 60) {
        var pubkeyBuffer = keyPair.publicKey
        ...
        privkey = libs.ethUtil.bufferToHex(keyPair.privateKey);
    } else if (isBch) {
    ...
    console.log('pubkey', pubkey);return privkey
}
